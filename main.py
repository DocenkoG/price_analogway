...
# import xlrd                          # для .xls
from price_tools import getCellXlsx, getCell, nameToId, currencyTypeX, sheetByName, sheetByNum
import csv
...
def getXlsxString(sh, i, in_columns_j):
    ...
    for item in in_columns_j.keys():
        ...
        if item in ('закупка', 'продажа', 'цена2', 'цена1'):
            ...
            if (sss.find('запросу') >= 0 or sss.find('ask') >= 0):
                impValues[item] = '0.1'
            else:
...
def convert_excel2csv(cfg):
    ...
    if cfg.has_option('basic', 'sheetname'):
    ...
    if cfg.has_option('basic', 'sheetnum'):
        sheetNum = cfg.getint('basic', 'sheetnum')
        log.debug('Reading file ' + priceFName)
        book, sheet = sheetByNum(fileName=priceFName, sheetNum=sheetNum)
    ...
    for i in range(1, sheet.max_row + 1):                                        # xlsx
        ...
        try:
            ...
            if (sheet.cell(row=i, column=in_cols_j['группа']).font.b is True and
                    sheet.cell(row=i, column=in_cols_j['цена1']).value is None):  # группа
                grp = impValues['группа']
                continue
            elif (impValues['код_'] == '' or
                  impValues['код_'] == 'Reference'):  # лишняя строка
                continue
            ...
            for outColName in out_template.keys():
                ...
                if ((outColName == 'закупка') or (outColName == 'продажа')) and ('*' in shablon):
                    p = shablon.find("*")
...
def main(dealerName):
    ...
    if os.path.exists('getting.cfg'):
        ...
        if not ((rc_download is True) or is_file_fresh(filename_new, int(cfg.get('basic', 'срок годности')))):
            return False
    for cfgFName in os.listdir("."):
        if cfgFName.startswith("cfg") and cfgFName.endswith(".cfg"):
            ...
            if (rc_download is True) or is_file_fresh(filename_in, int(cfg.get('basic', 'срок годности'))):
